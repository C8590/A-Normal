...
COMMAND_MATRIX: list[dict[str, Any]] = [
    ...
    {"category": "Basic", "command": "inspect-realism-data", "purpose": "Inspect optional A-share data realism CSV files.", "required_args": "-", "common_example": "python -m ashare_alpha inspect-realism-data", "output_location": "stdout", "safety_note": "Reads only local optional CSV files; no network or broker access."},
    {"category": "Basic", "command": "check-trading-calendar", "purpose": "Summarize open dates from optional trade_calendar.csv.", "required_args": "--start --end", "common_example": "python -m ashare_alpha check-trading-calendar --start 2026-01-01 --end 2026-03-31", "output_location": "stdout", "safety_note": "Reads only local calendar data and does not change research logic."},
    ...
    {"category": "数据源", "command": "run-realdata-offline-drill", "purpose": "运行 v0.3 离线真实数据演练链路。", "required_args": "--spec", "common_example": "python -m ashare_alpha run-realdata-offline-drill --spec configs/ashare_alpha/realdata/tushare_like_offline_drill.yaml", "output_location": "outputs/realdata/...", "safety_note": "只使用离线 fixture/cache，不调用外部 API、不接券商、不下单。"},
    {"category": "数据源", "command": "show-realdata-drill", "purpose": "查看已保存的离线真实数据演练结果。", "required_args": "--path", "common_example": "python -m ashare_alpha show-realdata-drill --path outputs/realdata/DRILL_ID/drill_result.json", "output_location": "stdout", "safety_note": "只读展示本地报告。"},
    ...
    {"category": "导入与质量", "command": "build-adjusted-bars", "purpose": "生成可审计的 raw/qfq/hfq 复权行情。", "required_args": "--date 或 --start --end", "common_example": "python -m ashare_alpha build-adjusted-bars --date 2026-03-20 --adj-type qfq", "output_location": "outputs/adjusted/...", "safety_note": "只读本地 CSV，不改变 pipeline 默认逻辑，不联网、不接券商、不下单。"},
    ...
    {"category": "研究", "command": "compute-factors", "purpose": "计算日频因子，可显式选择 raw/qfq/hfq 价格源。", "required_args": "--date", "common_example": "python -m ashare_alpha compute-factors --date 2026-03-20 --price-source qfq", "output_location": "outputs/factors/...", "safety_note": "默认 raw；qfq/hfq 只读取本地复权数据，不改变 pipeline/backtest 默认逻辑，不联网。"},
    {"category": "研究", "command": "compare-factor-price-sources", "purpose": "比较两个 factor price_source 版本的差异。", "required_args": "--date", "common_example": "python -m ashare_alpha compare-factor-price-sources --date 2026-03-20 --left raw --right qfq", "output_location": "outputs/factors/compare_...", "safety_note": "仅生成研究比较报告，不构成投资建议，不下单。"},
    ...
    {"category": "回测与报告", "command": "run-backtest", "purpose": "Run local research backtest with optional raw/qfq/hfq valuation price source.", "required_args": "--start --end", "common_example": "python -m ashare_alpha run-backtest --start 2026-01-05 --end 2026-03-20 --price-source qfq", "output_location": "outputs/backtests/...", "safety_note": "Default remains raw; execution constraints use raw daily bars; no broker connection or live orders."},
    {"category": "回测与报告", "command": "compare-backtest-price-sources", "purpose": "Compare two backtest valuation price sources such as raw vs qfq.", "required_args": "--start --end", "common_example": "python -m ashare_alpha compare-backtest-price-sources --start 2026-01-05 --end 2026-03-20 --left raw --right qfq", "output_location": "outputs/backtests/compare_...", "safety_note": "Research comparison only; execution constraints stay raw; no automatic orders."},
    {"category": "回测与报告", "command": "adjusted-research-report", "purpose": "Build a unified raw/qfq/hfq factor and backtest research comparison report.", "required_args": "--date --start --end", "common_example": "python -m ashare_alpha adjusted-research-report --date 2026-03-20 --start 2026-01-05 --end 2026-03-20", "output_location": "outputs/adjusted_research/...", "safety_note": "Research valuation comparison only; execution constraints remain raw; no external API, broker connection, or live orders."},
    ...
    {"category": "Frontend", "command": "build-frontend", "purpose": "生成本地只读静态 Research Frontend。", "required_args": "-", "common_example": "python -m ashare_alpha build-frontend --update-latest", "output_location": "outputs/frontend/...", "safety_note": "只读扫描 outputs，不联网，不执行命令，不修改研究逻辑。"},
    {"category": "Frontend", "command": "serve-frontend", "purpose": "用 Python http.server 服务已生成的静态前端目录。", "required_args": "--dir", "common_example": "python -m ashare_alpha serve-frontend --dir outputs/frontend/latest", "output_location": "local static server", "safety_note": "只服务静态文件，不提供 API，不接券商接口。"},
