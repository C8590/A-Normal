...
def _render_md(report: CandidateSelectionReport) -> str:
    ...
    lines = [
        ...
        *(["- gate_report_path: " + report.gate_report_path] if report.gate_report_path else []),
        *(["- gate_decision: " + (report.gate_decision or "-")] if report.gate_report_path else []),
        *(["- gate_message: " + (report.gate_message or "-")] if report.gate_report_path else []),
        "",
        "## 2. 候选排名",
